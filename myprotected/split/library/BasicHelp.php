...
class BasicHelp extends BasicPrinter
{
				...
				switch($params['type'])
				{
					...
					case 'shopOrder':
					{
						$result .= "<button class='rzh r-z-h-s-create' type='button' onclick=\"loadPage('$parent','$alias',$id,$item_id,'cardEdit',{});\" title='Редактировать'>Редактировать &nbsp;&nbsp;&nbsp;<span></span></button>
							<button class='rzh r-z-h-s-create' type='button' onclick=\"open_new_modal('sendProps',{orderId:$item_id});\" title='Отправить реквизиты'>
							Отправить реквизиты &nbsp;&nbsp;&nbsp;<span></span></button>
						";
		...
		public function getCardEditHeader($params=array())
		{
			...
			if($alias != "global-settings" && $alias != "shop-manage" && $alias != "profile" && $alias !="my-accounts")
			{
			...
			if(isset($params['type']))
			{
				switch($params['type'])
				{
					...
					case 'my-accounts':
					{
						$result .=	"<button class='rzh r-z-h-s-create' type='button' onclick=\"submit_save_form(3);\" title='Сохранить и перейти к просмотру'>Сохранить &nbsp;&nbsp;&nbsp;<span></span></button>";
						break;
					}
					default: break;
				}
				...
				foreach($params['tableColumns'] as $columnHeader => $columnParams)
				{
					switch($columnParams['type'])
					{
						...
						case 'image':
						{
							...
							$result .= "<td class='image'><a class='theater' href='$image_full_path' title='".str_replace("'","`",$item['name'])."' rel=''><img alt='БЕЗ КАРТИНКИ' style='max-width:200px; height:auto;' src='$image_path'></a></td>";
							break;
						}
						case 'update_cat_pos':
						{
