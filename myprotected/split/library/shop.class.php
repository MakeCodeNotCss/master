	...
	public function getShopDeliverDetails($id)
	{
		...
		public function getShopOrders($params=array(),$typeCount=false)
		{
			...
			if(!$typeCount)
			{
				...
				$query = "SELECT M.id, M.author_id, M.user_id, M.paid_status, M.pay_method, M.delivery_method, M.products_quant, M.sum, M.dateCreate, M.client_name, M.client_fname, M.prop_status,  
		...
		public function getQuickOrderDetails($id)
		{
			$query = "SELECT * FROM [pre]shop_quick_orders WHERE `id`='$id' LIMIT 1";
			$result = $this->rs($query);
		...
		public function getQuickOrderView($quickCart, $params=array())
		{
			$str = "";
			...
			$prod_id = $quickCart['prod_id'];
			
			$query = "SELECT id,name,sku,code,price 
						FROM [pre]shop_products 
						WHERE `id`=$prod_id 
						LIMIT 1";

			$prodMassive = $this->rs($query);

			$prodMassive = $prodMassive[0];

					
				$curr_price = $prodMassive['price'];
				$curr_name = $prodMassive['name'];
			
				$curr_sum = $curr_price*$quickCart['prod_quant'];
			
			$str .= "
					<tr class='trcolor' id='$iid'>
					";
					
					$str .= "<td>".$curr_name."</td>";
					$str .= "<td>".$prodMassive['sku']."</td>";
					$str .= "<td>".$quickCart['prod_quant']."</td>";
					$str .= "<td>".$curr_price."</td>";
					$str .= "<td>".$curr_sum."</td>";
			...
			$str .= "
					<table class='intotal'>
						<tr>
							<th>Сумма:		<span>".$curr_sum."</span> грн.</th>
						</tr>
					</table>
					";
			...
			$str .= "
					<table class='intotal'>
						<tr>
							<td>Имя:</td>
							<td>".$quickCart['user_name']."</td>
						</tr>
						<tr>
							<td>Телефон:</td>
							<td>".$quickCart['user_phone']."</td>
						</tr>
						<tr>
							<td>ID:</td>
							<td>".$quickCart['user_id']."</td>
						</tr>
					</table>
					";
